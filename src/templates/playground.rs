...
<head>
    ...
    <title>ClawProof — free zkML proofs</title>
    <style>
...
<pre>curl -X POST https://clawproof.onrender.com/prove \
  ...
  -d '{"model_id":"sentiment","input":{"text":"I love this product"}}'</pre>
...
function renderInputs() {
    ...
    if (m.input_type === 'text') {
        area.innerHTML = `<div class="form-group"><label class="form-label">Input text</label><textarea id="text-input" placeholder="Enter text to classify..."></textarea></div>`;
    } else if (m.fields) {
        ...
        area.innerHTML = `<div class="form-group"><label class="form-label">Raw input vector (JSON array of ${m.input_dim} integers)</label><textarea id="raw-input" placeholder="[0, 1, 2, ...]" rows="3"></textarea></div>`;
    }
}
